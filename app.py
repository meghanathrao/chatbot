...
if st.session_state["show_sidebar"]:
    ...
    username = st.sidebar.text_input("Username", "Guest")
    menu_selection = st.sidebar.radio("Navigate", ["New Chat", "History"], index=0)

    if menu_selection == "New Chat":
        st.sidebar.success("Start a new conversation!")
    elif menu_selection == "History":
        st.sidebar.info("View past conversations.")

if st.session_state["show_sidebar"]:
    if "username" in locals():
        st.write(f"Welcome, {username}!")

    if "menu_selection" in locals():
        if menu_selection == "New Chat":
            st.write("This is the New Chat section.")
        elif menu_selection == "History":
            st.write("This is the History section.")
...
def find_best_match(user_input):
    ...
    for intent in intents:
        for pattern in intent["patterns"]:
            ...
            if common_words:
                score = len(common_words) / max(len(pattern_words), len(input_words))
...
def handle_input():
    if st.session_state.user_input and st.session_state.user_input.strip():
        ...
        bot_response = get_response(best_match) if best_match else "I'm not sure how to respond to that."
...
def main():
    ...
    st.text_input("Message", key="user_input", on_change=handle_input, 
                 label_visibility="visible", placeholder="Type your message here...")
